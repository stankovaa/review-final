...
<style>
    ...
    /* Selectbox and input styling — white background, dark text */
    ...
    /* Text inputs (including disabled) — white background, dark text */
...
def welcome_dialog():
    ...
    st.markdown(
        ...
        "⚠️ This app is best viewed on a desktop or laptop."
    )
    st.markdown(
    ...
    with logo_col:
        st.markdown(
            ...
            f'<img src="data:image/png;base64,{_logo_b64}" style="max-width:80px;width:100%;">'
            f'</div>',
            unsafe_allow_html=True,
        )
...
if founders_pct + historical_pct + new_pct > 100.0:
    st.sidebar.error("Ownership exceeds 100%. Please reduce Founders or Historical investors")
...
with st.sidebar.container():
    st.markdown(
        """
        <div style="color: #6b6b6b; line-height: 0.9;">
...
_r = lambda arr: np.round(arr / 1e6, 1) * 1e6
...
# Round all graph values to 1 decimal place (in millions, i.e. round after /1e6)

exit_vals         = _r(exit_vals)
sell_today_vals   = _r(sell_today_vals)
raise_sell_vals   = _r(raise_sell_vals)
risked_vals       = _r(risked_vals)
...
# --- Output graphs --------------------------------------------------------
st.markdown("<h1 style='font-size:2.5rem;font-weight:700;'>Founder Proceeds by Exit Valuation <sup style='font-size:0.3em;vertical-align:super;'>6</sup></h1>", unsafe_allow_html=True)
...
# Round to display units (nearest 1m) so the gap is consistent with what the user sees
_desired_display            = round(desired / 1e6)
_risk_adjusted_display      = round(risk_adjusted_proceeds / 1e6)
_risk_gap_display           = _desired_display - _risk_adjusted_display  # matches what user sees
...
verdict = (
        f"Once Risk ⁸ is priced in, your raise &amp; sell proceeds are worth "
        f"<span class='trachet-highlight'>{currency}{_risk_adjusted_display}m</span> "
        f"(at post {new_round_name} "
        f"<span class='trachet-highlight'>{currency}{round(breakeven_raise_sell / 1e6)}m</span>). That is "
        f"<span class='trachet-highlight'>{currency}{_risk_gap_display}m</span> "
...
fig1.update_layout(
    ...
    margin=dict(t=25, r=20, b=75, l=45)
)
    ...
    try:
        ...
        if chart_data is not None:
            try:
                ...
                cfg = st.secrets["airtable"]
                upload_url = (
                    f"https://content.airtable.com/v0/{cfg['base_id']}"
                    f"/{record['id']}/{urllib.parse.quote('Graphs')}/uploadAttachment"
                )
                payload = json.dumps({
                    "contentType": "application/pdf",
                    "filename":    "founder_proceeds.pdf",
